...
class _CartPageState extends State<CartPage> {
  ...
  Image _getImageFromBase64(String base64String) {
    ...
    return Image.memory(bytes, width: 150, height: 150, fit: BoxFit.cover);
  }

  void _goToConfirmOrderPage() {
    List<CartItem> selectedItems =
        cartItems.where((item) => item.selected).toList();
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => ConfirmOrderPage(selectedItems: selectedItems),
      ),
    );
  }
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        item.image.isNotEmpty
                            ? _getImageFromBase64(item.image)
                            : Image.network(
                              item.imageUrl,
                              width: 150,
                              height: 150,
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text("Đơn giá: ${moneyFormat(item.price)}"),
                              Row(
                                children: [
                                  IconButton(
            ...
            child: Row(
              ...
              children: [
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("Tổng số sản phẩm: ${_getTotalItems()}"),
                    Text(
                      "Tạm tính: ${_getTotalFormatted()}",
                      style: const TextStyle(fontWeight: FontWeight.bold),
                    ),
                  ],
                ),
                ElevatedButton(
                  onPressed: _goToConfirmOrderPage,
